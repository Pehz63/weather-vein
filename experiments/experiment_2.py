...
def eval_gp(env, sim_data, seed):
    ...
    for _ in range(100):
        opt.zero_grad()
        x = torch.cat([yaws, freewind, ws, wd]).unsqueeze(0)
...
def eval_kg_pfn(env, sim_data, seed, device):
    ...
    obs = env.reset(seed=seed, options=OPTIONS)
    freewind_raw = np.concatenate([obs['freewind_measurements'], obs['wind_speed'], obs['wind_direction']])
...
# Main loop
# ============================================================
os.makedirs(f'./experiment_2', exist_ok=True)
RESULTS_CSV = './experiment_2/results.csv'
...
for layout in LAYOUTS:
    ...
    if not os.path.exists(f'../data/{layout}.csv'):
        pfn_data = collect_sim_data(env, PFN_SAMPLES, seed=27)
    else:
        pfn_data = pd.read_csv(f'../data/{layout}.csv')
    ...
    for seed_offset in range(args.n_seeds):
        ...
        print(f"Random={rand_score:.1f}, GP={gp_score:.1f}, KG-PFN={pfn_score:.1f}")
...
methods = ['random', 'gp', 'kg_pfn']
labels = ['Random', 'GP', 'PFN4BO']
...
plt.tight_layout()
plt.savefig('./experiment_2/baseline_barchart.png', bbox_inches='tight')
